...
export const HowItWorks = () => {
  return (
    <section className="mx-auto max-w-6xl px-6 py-20 text-center">
      <motion.h2
        className="font-title text-surface-on mb-12 text-3xl font-semibold md:text-4xl"
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
        {steps.map((step, index) => (
          <motion.div
            key={step.title}
            className="bg-surface-container-low elevation-1 flex flex-col items-center rounded-xl p-8 text-center"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <h3 className="font-title text-surface-on mb-3 text-xl font-semibold">
              {step.title}
            </h3>
            <p className="text-surface-on-muted text-base">
              {step.description}
            </p>
          </motion.div>
        ))}
